...
struct ContentView: View {
    ...
    @State var isSymboleAnimateing: Bool = false
    var body: some View {
        VStack(spacing: 16){
            Text("NV AI Voice Assistant")
                .font(.title2)
    ...
    var overlayView: some View {
        ...
        case .processingSpeech:
            Image(systemName: "brain")
                .symbolEffect(.bounce.up.byLayer, options: .repeating, value: isSymboleAnimateing)
                .font(.system(size: 128))
                .onAppear{ isSymboleAnimateing = true }
                .onDisappear{ isSymboleAnimateing = false }
        default: EmptyView()
        }
    }
